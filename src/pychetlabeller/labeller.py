...
class LabelDataset(object):
        ...
        for line in csv.reader(label_file):
            try:
                ...
                if tool == 'circle':
                    ...
                    label_name = self.labelmap[[x['object_id'] for x in self.labelmap].index(label_id)]['object_name']
                    self.add(
                        LabelRectangle(int(line[5]), float(line[1]), float(line[2]), float(line[3]), float(line[4]),
                                       label_name=label_name))
            except ValueError:
    ...
    def find(self, label_shape_id):
        """find a LabelShape given by ID"""
        results = [datum for datum in self.data if datum.id == label_shape_id]
        if len(results):
            return results[0]
        return None
...
class ToolCircle(Tool):
    ...
    def click(self, parent, button, release=False):
        ...
        if modifiers == QtCore.Qt.ShiftModifier and button == QtCore.Qt.LeftButton:
            ...
            parent.add_datum(LabelCircle(self.label, point[0], point[1], self.radius,
                                         label_name=
                                         self.labelmap[[x['object_id'] for x in self.labelmap].index(self.label)][
                                             'object_name']))
    ...
    def wheel(self, parent, QWheelEvent):
        ...
        parent.update()
        super(ToolCircle, self).wheel(parent, QWheelEvent)

    def paint(self, parent, QPainter, QStyleOptionGraphicsItem, QWidget):
        QPainter.drawEllipse(self.position.x() - self.radius, self.position.y() - self.radius, 2 * self.radius,
                             2 * self.radius)
...
class ToolRectangle(Tool):
    ...
    def click(self, parent, button, release=False):
        ...
        if modifiers == QtCore.Qt.ShiftModifier and button == QtCore.Qt.LeftButton:
            ...
            if self.mode == ToolRectangle.MODE_CENTRE:
                point = (self.position.x() - self.dx // 2, self.position.y() - self.dy // 2)
            parent.add_datum(LabelRectangle(self.label, point[0], point[1], self.dx, self.dy, label_name=
            self.labelmap[[x['object_id'] for x in self.labelmap].index(self.label)]['object_name']))

    def wheel(self, parent, QWheelEvent):
        delta = QWheelEvent.delta()
        modifiers = QtWidgets.QApplication.keyboardModifiers()
        if modifiers == QtCore.Qt.ShiftModifier:  # 1-pixel delta mode
            if self.resize_dim & ToolRectangle.RESIZE_X:
                self.dx = max(1, self.dx + np.sign(delta) * self.size_scroll_delta_precise)
            if self.resize_dim & ToolRectangle.RESIZE_Y:
        ...
        parent.update()
        super(ToolRectangle, self).wheel(parent, QWheelEvent)

    def paint(self, parent, QPainter, QStyleOptionGraphicsItem, QWidget):
        if self.mode == ToolRectangle.MODE_EDGE:
            QPainter.drawRect(self.position.x(), self.position.y(), self.dx, self.dy)
        elif self.mode == ToolRectangle.MODE_CENTRE:
    ...
    def key_down(self, parent, event):
        ...
        if key == QtCore.Qt.Key_Backspace and len(label_dataset.data):
            ...
            self.resize_dim = ToolRectangle.RESIZE_X
        elif key == QtCore.Qt.Key_A:
            self.resize_dim = ToolRectangle.RESIZE_Y
        else:
            keystr = str(key).lower()
            shortcuts = [x['keyboard_shortcut'] for x in self.labelmap]
            if keystr and keystr in shortcuts:
...
class ToolTransformView(Tool):
    def __init__(self):
        super(ToolTransformView, self).__init__()
...
class LabelRectangle(LabelShape):
    ...
    def svg_shape(self):
        ...
        x, y, dx, dy = self.get_rect_data()
        return svgwrite.shapes.Rect(insert=(x, y), size=(dx, dy), stroke=svgwrite.rgb(r, g, b, 'RGB'),
...
class LabelCircle(LabelShape):
    ...
    def svg_shape(self):
        """return the SVG shape that this object represents"""
        r, g, b = my_colormap[self.label]
        return svgwrite.shapes.Circle(center=(self.shape.x, self.shape.y), r=self.shape.radius,
                                      stroke=svgwrite.rgb(r, g, b, 'RGB'), fill=svgwrite.rgb(r, g, b, 'RGB'))
...
class SelectDropType(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        msg_box.setText('Which folder is this?')
        msg_box.addButton(QtWidgets.QPushButton('Cancel'), QtWidgets.QMessageBox.RejectRole)
        msg_box.addButton(QtWidgets.QPushButton('Labels'), QtWidgets.QMessageBox.NoRole)
        msg_box.addButton(QtWidgets.QPushButton('Images'), QtWidgets.QMessageBox.YesRole)
        self.selection = msg_box.exec_()
...
class ObjectDrawPanel(QtWidgets.QGraphicsPixmapItem):
    ...
    def __init__(self, pixmap=None, parent=None, scene=None, tool='circle', labelmap=None, penwidth=1):
        ...
        self.tool_last = ToolTransformView()
        if tool == 'circle':
            self.tool = ToolCircle()
        elif tool == 'rectangle':
            self.tool = ToolRectangle()
        else:
    ...
    def paint(self, QPainter, QStyleOptionGraphicsItem, QWidget):
        ...
        for datum in label_dataset.data:
            if datum is self.highlighted_datum:
                QPainter.setBrush(self.highlight_brushes[datum.label])
            else:
                QPainter.setBrush(self.save_brushes[datum.label])
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def dropEvent(self, QDropEvent):
        ...
        if QDropEvent.mimeData().hasUrls:
            ...
            QDropEvent.accept()
            prompt_user = SelectDropType()
            prompt_user.show()
            if prompt_user.selection == 1:
                urls = QDropEvent.mimeData().urls()
                assert len(urls) == 1
                self.labelFolder = str(urls[0].toString()[7:])
            if prompt_user.selection == 2:
                urls = QDropEvent.mimeData().urls()
                assert len(urls) == 1
                image_folder = str(urls[0].toString()[7:])
                self.openImageDirectory(folder_image=image_folder)
        else:
            QDropEvent.ignore()
    ...
    def change_brightness_contrast(self):
        ...
        pixmap = adjust_pixmap(self.imagePanel.defaultColorPixmap, brightness=b_value, contrast=c_value)
    ...
    def init_image(self, pixmap):
        ...
        self.ui.graphicsView.setSceneRect(0, 0, self.ui.graphicsView.size().width() - 10,
    ...
    def load_image(self, image_path):
        ...
        if self.firstImage or pixmap.width() != self.original_size[0] or pixmap.height() != self.original_size[1]:
        ...
        # @TODO: First image does not auto-load annotations because UI is not init yet @priority: low
    ...
    def changeImage(self, text):
        ...
        self.load_image("%s/%s" % (self.folder_image, str(self.ui.imageComboBox.currentText())))
        self.setWindowTitle(
            "{} - Pychet Object Annotator [{}]".format(self.ui.imageComboBox.currentText(), self.tool_str))
    ...
    def openImageDirectory(self, folder_image=None):
        ...
        open_directory = self.folder_image or self.default_directory
        self.folder_image = folder_image or str(
            QtWidgets.QFileDialog.getExistingDirectory(self, "Open directory", open_directory))
        # Grab images and set up combobox
        self.images = sorted([x for x in os.listdir(self.folder_image) if
                              os.path.splitext(x)[-1].lower() in ['.png', '.jpg', '.jpeg', '.tiff', '.bmp']])
    ...
    def setLabelDirectory(self, dir_path=None):
        ...
        if dir_path:
            ...
            open_directory = self.folder_image or '.'
            label_folder_choice = str(QtWidgets.QFileDialog.getExistingDirectory(self, "Open directory", open_directory))
            if label_folder_choice:
                self.labelFolder = label_folder_choice
...
Pick image directory (containing .jpg or .png files) and start labelling! Labels saved as .csv in the same parent folder
 as the images, under folder labels
...
def convertQImageToMat(incoming_image):
    """  Converts a QImage into an opencv MAT format  """
    # solution in https://stackoverflow.com/questions/19902183/qimage-to-numpy-array-using-pyside?noredirect=1&lq=1
    incoming_image = incoming_image.convertToFormat(QtGui.QImage.Format.Format_RGB32)
...
def convert_mat_to_QImage(im, copy=False):
    if im is None:
        return QtGui.QImage()
...
def adjust_pixmap(pixmap, brightness=0, contrast=0):
    ...
    im = convert_mat_to_QImage(imcv.astype('uint8'))
...
def parse_args():
    ...
    parser.add_argument('--labelmap', dest='labelmap', default=None,
                        help='JSON file for annotation labels; default labels: 0,1,...,9; 0 for background; use number keys to set desired label')
